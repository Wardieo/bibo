    ...
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="report-dialog" role="dialog" aria-modal="true" aria-labelledby="report-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <label className="field-label">
          ...
          <textarea value={details} onChange={(event) => setDetails(event.target.value)} placeholder="Tell our moderators what they should know." rows={4} />
        </label>
        <label className="urgent-check">
          <input type="checkbox" checked={urgent} onChange={(event) => setUrgent(event.target.checked)} />
          <span>This involves immediate danger, threats, or suspected child sexual exploitation.</span>
        </label>
        <p className="privacy-line">Your report is private and will be reviewed by our safety team.</p>
        <button className="primary-button report-submit" type="button" onClick={() => onSubmit(category, details, urgent)}>Submit report <span aria-hidden="true">-&gt;</span></button>
      </section>
    </div>
  )
}
