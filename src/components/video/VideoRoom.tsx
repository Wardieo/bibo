...
export function VideoRoom({ onLeave, onNext }: VideoRoomProps) {
  ...
  const [phase, setPhase] = useState<
    "setup" | "searching" | "connecting" | "live"
  >("setup");
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (!cancelled && match?.status === "matched" && match.room_id)
          await connectToRoom(match.room_id);
      } catch (error) {
  ...
  const startMatching = async () => {
    ...
    try {
      ...
      setPhase("setup");
      setNotice(
        describeError(error, "Unable to enter matchmaking.").replace(
          /^Error:\s*/i,
          "",
        ),
      );
      ...
      <section className="room-app setup-room" aria-labelledby="room-title">
        <aside className="country-rail" aria-label="Choose a country">
          ...
          {[
            ...
            >
              <span>{option === "International" ? "◎" : "○"}</span>
              {option}
            </button>
          ))}
        ...
        <main className="setup-content">
          ...
          <div className="room-brand">
            ...
            <strong>bibo</strong>
            <span className="room-tag">random video chat</span>
          </div>
          <div className="setup-center">
            <p className="setup-kicker">
              {country} <span className="status-dot" />
            </p>
            <h1 id="room-title">
              Who do you want
              <br />
            ...
            <p className="setup-subtitle">
              Choose your room size, then we will find a conversation waiting
              for you.
            </p>
            <div
              className="capacity-options"
              role="radiogroup"
              aria-label="Number of people to meet"
            >
              {[1, 2, 3].map((size) => (
                <button
                  type="button"
                  ...
                  key={size}
                  onClick={() => setGroupSize(size as 1 | 2 | 3)}
                >
                  <strong>{size}</strong>
                  <span>{size === 1 ? "1 on 1" : `you + ${size} people`}</span>
                  <i />
                </button>
              ))}
            </div>
            ...
            <p className="setup-privacy">
              Your camera and microphone stay off until you enter. No public
              broadcasting.
            </p>
            {notice && (
  ...
  if (phase === "searching" || phase === "connecting") {
    return (
      <section
        className="room-shell room-app queue-room"
      ...
      >
        <div className="room-topbar">
          ...
          <div>
            ...
            <h1 id="queue-title">Waiting for your room</h1>
          </div>
          <div className="room-status">
            <span className="waiting-dot" /> Finding {groupSize}{" "}
        ...
        <div className={`video-grid queue-grid capacity-${groupSize}`}>
          <article className="video-tile lavender queue-local">
            ...
            {previewStream ? (
              ...
              <div className="avatar-placeholder">YO</div>
            )}
        ...
        <div className="queue-footer">
          ...
          <p>
            {phase === "searching"
              ? `Waiting for ${groupSize} ${groupSize === 1 ? "other person" : "other people"} who chose the same room size.`
  ...
  return (
    <section className="room-shell room-app" aria-labelledby="room-title">
      <div className="room-topbar">
        ...
        <div className="room-status">
          <span className="status-dot" /> {visibleParticipants.length} /{" "}
          {groupSize + 1} people &middot;{" "}
          {visibleParticipants.length === groupSize + 1
            ? "Room is full"
            : `Looking for ${groupSize + 1 - visibleParticipants.length} more`}
        </div>
      </div>
      <div className="live-room-content">
        <div
          className={`video-grid capacity-${groupSize} participants-${visibleParticipants.length}`}
          key={participantVersion}
        >
          ...
          {Array.from({
            length: Math.max(0, groupSize + 1 - visibleParticipants.length),
          }).map((_, index) => (
            <article className="video-tile loading-tile" key={`empty-${index}`}>
              <span className="loading-ring" />
              <div className="tile-meta">
                <strong>Waiting for someone</strong>
