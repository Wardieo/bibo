...
export async function ensureSession() {
  ...
  if (error || !data.session) throw error ?? new Error('Enable Supabase Anonymous Auth to start matchmaking.')
  return data.session
}

export async function joinMatchmaking(desiredPeople: DesiredPeople, mode: MatchMode = 'video'): Promise<MatchResult> {
  const client = requireSupabase()
  const { data, error } = await client.rpc('join_matchmaking', { requested_people: desiredPeople, requested_mode: mode })
...
export function subscribeToMatch(userId: string, onChange: () => void) {
  ...
  return client
    ...
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'match_queue', filter: `user_id=eq.${userId}` }, onChange)
    .subscribe()
}
...
export async function getCurrentMatch() {
  ...
  if (userError || !userData.user) throw new Error('Sign in is required before matchmaking.')
