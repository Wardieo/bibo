alter table public.match_queue
  add column if not exists match_mode text not null default 'video'
  check (match_mode in ('video', 'text'));

alter table public.rooms
  add column if not exists room_type text not null default 'video'
  check (room_type in ('video', 'text'));

create index if not exists match_queue_mode_lookup
  on public.match_queue (match_mode, desired_people, status, created_at);
